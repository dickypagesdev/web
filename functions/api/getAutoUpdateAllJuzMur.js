// getAutoUpdateAllJuzMur.js
import { ok } from "../_lib/db.js";
export async function onRequestGet({ env }){
  const { results } = await env.DB.prepare(`SELECT class_name AS kelas, from_date AS fromDate, to_date AS toDate FROM class_ranges_mur`).all();
  return ok(results||[]);
}

// aksesAutoUpdateAllJuzMur.js
...
export async function onRequestPost({ request, env }){
  ...
  await env.DB.prepare(`
    INSERT INTO class_ranges_mur (class_name, from_date, to_date, updated_at)
    VALUES (?, ?, ?, datetime('now'))
