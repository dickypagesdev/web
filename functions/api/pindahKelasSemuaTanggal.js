...
// Body: { kelasAsal, kelasTujuan, ids?:[], nises?:[], names?:[] }
...
export async function onRequest({ request, env }){
  ...
  let { kelasAsal, kelasTujuan, ids=[], nises=[], names=[] } = body || {};
  if (!kelasAsal || !kelasTujuan) return J(400,{error:"Wajib: kelasAsal & kelasTujuan"});
  ...
  try{
    ...
    srcObj.records = [...sRecs].sort((a,b)=>String(a.tanggal).localeCompare(String(b.tanggal)));
    dstObj.records = [...dRecs].sort((a,b)=>String(a.tanggal).localeCompare(String(b.tanggal)));

    await ghPutJsonAgg(token, `absensi/${tujuan}.json`, dstObj, null, `pindah semua: ${asal} → ${tujuan}`);
    await ghPutJsonAgg(token, `absensi/${asal}.json`,   srcObj, null, `pindah semua: ${asal} → ${tujuan} (remove)`);
