...
// POST body: { asal, tujuan, startDate: "YYYY-MM-DD", ids?: number[], idMap?: { [oldId]: newId } }
// Pindahkan items untuk semua tanggal >= startDate
// ENV: GITHUB_TOKEN
...
export async function onRequest({ request, env }) {
  ...
  let { asal, tujuan, startDate, ids, idMap } = body||{};
  if (!asal || !tujuan || !isDate(startDate)) return json({success:false,error:"asal, tujuan, startDate(YYYY-MM-DD) wajib."},400);
  ...
  try {
    ...
    const msg = `pindahKelasMulaiTanggal: ${asal} → ${tujuan} sejak ${startDate}`;
    try {
