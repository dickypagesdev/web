...
// POST { kelas, key, keterangan }   // keterangan: "", SP1..SP4
// ENV: GITHUB_TOKEN
...
export async function onRequest({request,env}){
  ...
  let { kelas, key, keterangan } = body||{};
  kelas=normKelas(kelas); key=clean(key);
  keterangan = clean(keterangan||"").toUpperCase();
  ...
  const idx=arr.findIndex(s=>{
    if(!s) return false;
    const sid=String(s.id??""); const sn=clean(s.nama); const snis=clean(s.nis);
    return key===sid || key===snis || key===sn;
  });
  if(idx<0) return json({success:false,error:"Santri tidak ditemukan."},404);

  if(keterangan) arr[idx].keterangan = keterangan; else delete arr[idx].keterangan;

  const putUrl=`https://api.github.com/repos/${OWNER_REPO}/contents/${encodeURIComponent(kelas)}.json`;
  const bodyPut={message:`updateKeterangan: ${kelas} key=${key} → ${keterangan||"(kosong)"}`,content:b64enc(JSON.stringify(arr,null,2)),branch:BRANCH,sha};
  ...
  return json({success:true, kelas, key, keterangan, commit:js?.commit?.sha||null});
}
