...
// POST /api/updateKeteranganKelas
// Body: { kelas, keterangan, key: { id? | nis? | nama? } }
...
export async function onRequest({ request, env }) {
  ...
  let { kelas, keterangan, key } = body || {};
  if (!kelas || typeof keterangan !== "string" || !key || typeof key !== "object")
    return J(400, { error: "Wajib: kelas, keterangan, key{}" });
  ...
  try {
    ...
    arr[idx] = { ...arr[idx], keterangan: String(keterangan) };
    await ghPutJsonAgg(token, path, arr, null, `updateKeterangan: ${kelas} idx=${idx}`);
