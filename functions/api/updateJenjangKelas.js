...
// POST /api/updateJenjangKelas
// Body: { kelas, jenjang, key: { id? | nis? | nama? } }
...
export async function onRequest({ request, env }) {
  ...
  let { kelas, jenjang, key } = body || {};
  if (!kelas || !jenjang || !key || typeof key !== "object") return J(400, { error: "Wajib: kelas, jenjang, key{}" });
  ...
  try {
    ...
    arr[idx] = { ...arr[idx], jenjang: String(jenjang) };
    await ghPutJsonAgg(token, path, arr, null, `updateJenjang: ${kelas} idx=${idx}`);
