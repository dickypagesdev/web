...
// POST { kelas, key, jenjang }  // key boleh NIS/ID/Nama; jenjang: "" atau A1..A<JENJANG_MAX>
// ENV: GITHUB_TOKEN, JENJANG_MAX (default 34)
...
export async function onRequest({request,env}){
  ...
  let { kelas, key, jenjang } = body||{};
  kelas=normKelas(kelas); key=clean(key); jenjang=clean(jenjang||"");
  ...
  if(jenjang){
    ...
    if(!Number.isInteger(j)||j<1||j>JMAX) return json({success:false,error:`Jenjang di luar batas (1..${JMAX}).`},400);
    jenjang=`A${j}`;
  }
  ...
  // cari index by NIS/ID/Nama
  const idx = arr.findIndex(s=>{
    if(!s) return false;
    const sid = String(s.id??"");
    const sn  = clean(s.nama);
    const snis= clean(s.nis);
    return key===sid || key===snis || key===sn;
  });
  if(idx<0) return json({success:false,error:"Santri tidak ditemukan."},404);

  arr[idx].jenjang = jenjang; // bisa ""
  // PUT
  const putUrl=`https://api.github.com/repos/${OWNER_REPO}/contents/${encodeURIComponent(kelas)}.json`;
  const bodyPut={message:`updateJenjang: ${kelas} key=${key} → ${jenjang||"(kosong)"}`,content:b64enc(JSON.stringify(arr,null,2)),branch:BRANCH,sha};
  ...
  return json({success:true, kelas, key, jenjang, commit:js?.commit?.sha||null});
}
