...
export async function onRequest({ request, env }) {
  ...
  const r = await getJsonSmart("secure.json", env.GITHUB_TOKEN);
  if (!r.ok)  return json({ message:`Gagal ambil secure.json`, error:r.error }, r.status || 500);
  if (!r.exists) return json({ message:"secure.json tidak ditemukan." },404);

  const ok = (r.data?.adminPassword || "") === password;
  return json({ message: ok ? "Password admin valid." : "Password admin salah." }, ok ? 200 : 401);
}
