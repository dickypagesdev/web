...
const GITHUB_API = (ownerRepo) => withRef(`https://api.github.com/repos/${ownerRepo}/contents`);
...
export async function onRequest({ request, env }) {
  ...
  try {
    const res = await fetch(GITHUB_API(OWNER_REPO), { headers: ghHeaders(token) });
    ...
    if (!res.ok) {
      ...
      return json({ message: `Gagal fetch list kelas (${res.status})`, error: error.slice(0, 300) }, res.status);
    }
